...
export interface GridUtilities {
    ...
    updateManyCells: <T extends Cell>(
        grid: T[][],
        cells: T[],
        updateCell: (cell: T) => T
    ) => T[][];
    cloneGrid: <T extends Cell>(grid: T[][]) => T[][];
    printGrid: <T extends Cell>(
        grid: T[][],
        printCell: (cell: T) => string
    ) => void;
}
...
export const useGridUtilities = (): GridUtilities => {
    ...
    const getBoxNumber = (rowNumber: number, columnNumber: number): number => {
        const boxNumber =
            Math.floor(rowNumber / 3) * 3 + Math.floor(columnNumber / 3);
        return boxNumber;
    };

    const updateGridCell = <T>(
        grid: T[][],
        row: number,
        column: number,
        state: T
    ...
    const updateManyCells = <T extends Cell>(
        grid: T[][],
        cells: T[],
        updateCell: (cell: T) => T
    ): T[][] => {
    ...
    const cloneGrid = <T extends Cell>(grid: T[][]): T[][] => {
        return grid.map(row =>
            row.map(cell => JSON.parse(JSON.stringify(cell)) as T)
        );
    };

    const printGrid = <T extends Cell>(
        grid: T[][],
        printCell: (cell: T) => string
    ): void => {
        for (let r = 0; r < 9; r++) {
